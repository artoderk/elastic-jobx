...
public class JobBeanDefinitionParser extends AbstractBeanDefinitionParser {
    ...
    private String createJobConfiguration(final Element element, final ParserContext parserContext) {
        ...
        if (StringUtils.isEmpty(element.getAttribute("shardingTotalCount"))) {
        	factory.addConstructorArgValue("1");
        } else {
			factory.addConstructorArgValue(element.getAttribute("shardingTotalCount"));
        }
        
        if (StringUtils.isEmpty(element.getAttribute("cron"))) {
        	// TODO
        	factory.addConstructorArgValue("0 0 1 * * ?");
        } else {
        ...
        addPropertyValueIfNotEmpty("skipTimeStart", element, factory);
        addPropertyValueIfNotEmpty("skipTimeEnd", element, factory);
