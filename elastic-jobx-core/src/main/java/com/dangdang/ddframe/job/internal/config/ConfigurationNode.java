...
public final class ConfigurationNode {
    ...
    static final String SKIP_TIME_START = ROOT + "/skipStartTime";
    
    static final String SKIP_TIME_END = ROOT + "/skipEndTime";
     ...
     * @return 是否为作业分片策略路径
     */
    public boolean isShardingStrategyClassPath(final String path) {
        return jobNodePath.getFullPath(JOB_SHARDING_STRATEGY_CLASS).equals(path);
    }
     ...
     * 
     * @param path 节点路径
     * @return 是否为执行起始时间配置路径
     */
    public boolean isSkipTimeStartPath(final String path) {
        return jobNodePath.getFullPath(SKIP_TIME_START).equals(path);
    }
     ...
     * 
     * @param path 节点路径
     * @return 是否为跳过执行终点时间配置路径
     */
    public boolean isSkipTimeEndPath(final String path) {
        return jobNodePath.getFullPath(SKIP_TIME_END).equals(path);
    }
}
